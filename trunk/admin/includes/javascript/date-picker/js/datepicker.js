// datepicker by frequency-decoder.com. released under a non-commercial creative commons license
...
function datePicker(options) {
        ...
        o.setRangeLow = function(range) {
                range = datePickerController.dateFormat(range);
                if(range === 0) range = '';
        ...
        o.resize = function() {
                ...
                o.div.style.top   = pos[1] + (elem.offsetHeight + 3) + "px";
        ...
        o.setDateFromInput = function() {
                ...
                if(new Date( dates[y] + "/" + dates[m] + "/" + dates[d] ) == 'Invalid Date') {
                        o.date = new Date();
                        return;
                };
        ...
        o.returnFormattedDate = function() {
                ...
                if(!(o.disableDays[weekDay])) {
                        if(o.splitDate) {
                                document.getElementById(o.id+"-dd").value = o.format.replace('y','').replace('m','').replace('d',d).replace(/-/g,'');
                                document.getElementById(o.id+"-mm").value = o.format.replace('y','').replace('m',m).replace('d','').replace(/-/g,'');
                                elem.value = o.format.replace('y',yyyy).replace('m','').replace('d','').replace(/-/g,'');
                                ...
                                elem.value = o.format.replace('y',yyyy).replace('m',m).replace('d',d).replace(/-/g,o.dividor);
                                elem.focus();
                        }
        ...
        o.create();
        o.init();
        datePickerController.addEvent(window, 'resize', o.resize, true);
}
...
datePickerController = {
        ...
        create: function() {
                ...
                var regExp7 = /dividor-(dot|slash|space|dash)/g;        // the character used to divide the date
                var regExp8 = /no-locale/g;                             // do not attempt to detect the browser language

                for(var i=0, inp; inp = inputs[i]; i++) {
                        if(inp.className && inp.className.search(regExp6) != -1 && inp.type == "text" && inp.name) {
                                ...
                                if(inp.className.search(/split-date/) != -1) {
                                        if(document.getElementById(inp.id+'-dd') && document.getElementById(inp.id+'-mm') && document.getElementById(inp.id+'-dd').tagName == "INPUT" && document.getElementById(inp.id+'-mm').tagName == "INPUT") {
                                                options.splitDate = 1;
                                        };
                                };
                                ...
                                // What dividor to use, a "/", "-", "." or " "
                                if(inp.className.search(regExp7) != -1) {
                                        var dividor = inp.className.match(regExp7)[0].replace('dividor-','');
                                        switch(dividor.toLowerCase()) {
                                                case "dot":
                                                        options.dividor = ".";
                                                        break;
                                                case "space":
                                ...
                                // Datepicker is already created so reset it's defaults and continue
