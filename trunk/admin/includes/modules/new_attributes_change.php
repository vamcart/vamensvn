...
if(isset($_POST['category'])){
	$current_category = $_POST['current_product_id'];
	// Recieve all items in this category
	$items_query = "SELECT products_id FROM ".TABLE_PRODUCTS_TO_CATEGORIES." WHERE categories_id = " . $current_category;
	$items_result = vam_db_query($items_query);
   $items_matches = vam_db_num_rows($items_result);
   
  if ($items_matches) {
  		while ($items_line = vam_db_fetch_array($items_result)) {
  			//echo('<pre>'); print_r($items_line);  echo('</pre>');
  			$current_product_id = $items_line['products_id'];
  			//echo('<pre>'); print_r($current_product_id);  echo('</pre>');  			
  			ItemAttributsSetUp($current_product_id);
  			}
  } else {
 		echo "You have no items in selected category / Нет товаров в выбранной категории";
  } 	
} 
else{
