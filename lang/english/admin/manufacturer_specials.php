<?php
/* --------------------------------------------------------------
   $Id: category_specials.php 1125 2008-02-26 11:13:01Z VaM $   

   VaM Shop - open source ecommerce solution
   http://vamshop.ru
   http://vamshop.com

   Copyright (c) 2008 VaM Shop
   --------------------------------------------------------------
   based on:
   category_specials.php (c) 2005 WTM Technologies http://wtmtechnologies.com

   Released under the GNU General Public License
   --------------------------------------------------------------*/

define('HEADING_TITLE', 'Special Brands');

define('TABLE_HEADING_CATEGORY', 'Category');
define('TABLE_HEADING_SPECIAL_PRODUCT', 'Product(s)');
define('TABLE_HEADING_CATEGORY_DISCOUNT', 'Category Discount');
define('TABLE_HEADING_STATUS', 'Status');
define('TABLE_HEADING_ACTION', 'Action');

define('TEXT_SPECIALS_CATEGORY', 'Category:');
define('TEXT_SPECIALS_SPECIAL_PRICE', 'Special Price:');
define('TEXT_SPECIALS_DISCOUNT_TYPE', 'Discount Type:');
define('TEXT_SPECIALS_EXPIRES_DATE', 'Expiry Date:');
define('TEXT_SPECIALS_OVERRIDE', 'Override discount of already existing Specially priced products under the category.');
define('TEXT_SPECIALS_PRICE_TIP', '<b>Specials Notes:</b><ul><li>You can enter a percentage to deduct in the Specials Price field, for example: <b>20%</b></li><li>If you enter a new price, the decimal separator must be a \'.\' (decimal-point), example: <b>49.99</b></li><li>Leave the expiry date empty for no expiration</li></ul>');

define('TEXT_INFO_DATE_ADDED', 'Date Added:');
define('TEXT_INFO_LAST_MODIFIED', 'Last Modified:');
define('TEXT_INFO_DISCOUNT_TYPE', 'Discount Type:');
define('TEXT_INFO_DISCOUNT', 'Discount:');

define('TEXT_INFO_NEW_PRICE', 'New Price:');
define('TEXT_INFO_EXPIRES_DATE', 'Expires At:');
define('TEXT_INFO_STATUS_CHANGE', 'Status Change:');

define('TEXT_INFO_HEADING_DELETE_SPECIALS', 'Delete Special Category');
define('TEXT_INFO_DELETE_INTRO', 'Are you sure you want to delete the special category price?');

define('TABLE_HEADING_PRODUCTS', 'Products');
define('TABLE_HEADING_PRODUCTS_PRICE', 'Products Price');
define('TABLE_HEADING_PRODUCTS_SPECIAL_PRICE', 'Specials Price');
define('TABLE_HEADING_STATUS', 'Status');
define('TABLE_HEADING_ACTION', 'Action');

define('TEXT_INFO_DATE_ADDED', 'Date Added:');
define('TEXT_INFO_LAST_MODIFIED', 'Last Modified:');
define('TEXT_INFO_NEW_PRICE', 'New Price:');
define('TEXT_INFO_ORIGINAL_PRICE', 'Original Price:');
define('TEXT_INFO_PERCENTAGE', 'Percentage:');
define('TEXT_INFO_EXPIRES_DATE', 'Expires At:');
define('TEXT_INFO_STATUS_CHANGE', 'Status Change:');

define('TEXT_INFO_HEADING_DELETE_SPECIALS', 'Delete Special');
define('TEXT_INFO_DELETE_INTRO', 'Are you sure you want to delete the special products price?');

define('BUTTON_UPDATE', 'Update');
define('BUTTON_RE_ADD', 'Re-Add');
define('BUTTON_MAKE_SPECIAL', 'Make Special');
define('BUTTON_DELETE', 'Delete');

define('NEW_MANUFACTURER_DISCOUNT', 'New Manufacturer Discount');
?>