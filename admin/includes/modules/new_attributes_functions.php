<?php
/* --------------------------------------------------------------
   $Id: new_attributes_functions.php 899 2007-02-08 12:28:21 VaM $   
   ...
   (c) 2004 xt:Commerce (new_attributes_functions.php,v 1.8 2003/08/14); xt-commerce.com
  ...
  function checkAttribute($current_value_id, $current_pid, $current_product_option_id) {
    ...
    if ($isFound) {
      while($line = vam_db_fetch_array($result)) {
          ...
          global $$link;
        $dl_sql = vam_db_query("SELECT products_attributes_maxdays, products_attributes_filename, products_attributes_maxcount, products_attributes_is_pin  FROM ".TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD." WHERE products_attributes_id = '" . $line['products_attributes_id'] . "'") or die(mysqli_error($$link));
