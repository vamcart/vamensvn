...
if ($_GET['action']) {
	switch ($_GET['action']) {
		case 'new_order' :
			...
			$sql_data_array = array ('orders_id' => $orders_id, 'title' => '<b>Всего</b>:', 'text' => '0', 'value' => '0', 'class' => 'ot_total');

			$insert_sql_data = array ('sort_order' => MODULE_ORDER_TOTAL_TOTAL_SORT_ORDER);
			$sql_data_array = vam_array_merge($sql_data_array, $insert_sql_data);
			vam_db_perform(TABLE_ORDERS_TOTAL, $sql_data_array);

			$sql_data_array = array ('orders_id' => $orders_id, 'title' => '<b>Стоимость товара</b>:', 'text' => '0', 'value' => '0', 'class' => 'ot_subtotal');
...
<?php include(DIR_WS_MODULES . FILENAME_CUSTOMER_MEMO); ?>
      ...
      <tr>
        <td align="right" class="main"><input type="submit" class="button" onClick="this.blur();" value="<?php echo BUTTON_UPDATE; ?>"><?php echo ' <a class="button" onClick="this.blur();" href="' . vam_href_link(FILENAME_CUSTOMERS, vam_get_all_get_params(array('action'))) .'">' . BUTTON_CANCEL . '</a>'; ?></td>
      </tr></form>
<?php
	...
	switch ($_GET['action']) {
		...
		default :
			...
			if (is_object($cInfo)) {
				...
				if ($cInfo->customers_id != 1) {
					$contents[] = array ('align' => 'center', 'text' => '<a class="button" ' . ($cs_id != 0 ? 'onClick="alert(\'Сначала определите пользователя в группу Админ (кнопка Статус покупателя)!\nДоступ в админку можно настраивать только для пользователей, состоящих в группе Админ.\');"' : 'onClick="this.blur();"') . ' href="'.vam_href_link(FILENAME_ACCOUNTING, vam_get_all_get_params(array ('cID', 'action')).'cID='.$cInfo->customers_id).'">'.BUTTON_ACCOUNTING.'</a>');
				}
