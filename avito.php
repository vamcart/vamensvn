...
vam_yml_out('<?xml version="1.0" encoding="' . $charset .'"?' . '>');
vam_yml_out('<Ads target="Avito.ru" formatVersion="3">');
...
while ($products = vam_db_fetch_array($products_query)) {
  ...
  vam_yml_out('  <Ad>');
  vam_yml_out('<Id>' . $products['products_id'] . '</Id>');
  vam_yml_out('<Category>Спорт и отдых</Category>');
  vam_yml_out('<GoodsType>Другое</GoodsType>');
  vam_yml_out('<AdType>Товар приобретен на продажу</AdType>');
  vam_yml_out('<Address>Россия, Москва, Дмитровское шоссе, 60</Address>');
  vam_yml_out('<Latitude>55.852933</Latitude>');
  vam_yml_out('<Longitude>37.567438</Longitude>');
  vam_yml_out('<ManagerName>Менеджер по продажам</ManagerName>');
  vam_yml_out('<ContactPhone>+7 (XXX) XXX-XX-XX</ContactPhone>');
  vam_yml_out('<AllowEmail>Да</AllowEmail>');
  vam_yml_out('<AdStatus>Free</AdStatus>');
  
  vam_yml_out('  <Price>' . ($old_price > 0 && $products['status'] == 1  ? $old_price : $price) . '</Price>');

  vam_yml_out('  <Images>');
  if(vam_not_null($products['products_image'])) vam_yml_out('  <Image url="' . HTTP_SERVER . DIR_WS_CATALOG . DIR_WS_THUMBNAIL_IMAGES . urldecode($products['products_image']) . '"/>');

		$mo_images = vam_get_products_mo_images($products['products_id']);
        if ($mo_images != false) {
            foreach ($mo_images as $img) {
                vam_yml_out('  <Image url="' . HTTP_SERVER . DIR_WS_CATALOG . DIR_WS_POPUP_IMAGES . urldecode($img['image_name']) . '"/>');
            }
        }
  vam_yml_out('  </Images>');
